 ...
 */
package com.github.khazrak.jdocker.docker_api_1_24.exec;
...
public class ExecCreate {
    ...
    private static Logger logger = LoggerFactory.getLogger(ExecCreate.class);
    ...
    @Test
    public void create() {
        final String path = "/v1.24%2Fcontainers%2Fmongo%2Fexec";

        List<String> commands = Arrays.asList("date");
        //List<String> commands = Arrays.asList("grep","-Rls","'.conf'","/");

        ExecCreateRequest request = ExecCreateRequest.builder().attachStdOut(true).cmd(commands).build();

        String containerId = "mongo";
        String id = client.createExec(containerId, request);

        assertThat(id).isNotNull();
        assertThat(id).isEqualTo("2cf82cdcaca6da3ed97e9099fd180212989da593c254377decd6eecbb00c8179");
